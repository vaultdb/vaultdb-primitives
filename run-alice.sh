...
#example invocation: ./run-alice.sh  Millionaires 127.0.0.1 54321
if [$# -ne 3];
then
    echo "usage: ./run-alice.sh classname hostname port";
     exit -1;
fi
...
java -cp $JAVACPP_JAR:$JAVACPP_WORKING_DIRECTORY  org.vaultdb.compiler.emp.generated.$CLASSNAME 1 $HOST $PORT
